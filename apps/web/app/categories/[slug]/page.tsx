...
// AC-1 — browsing into a category shows every design assigned to it. Routes are slug-based
// (spec §5) but the designs endpoint is id-based, so this resolves slug → id first via the
// categories list (no dedicated "get by slug" endpoint exists — small enough list not to need one).
  ...
  useEffect(() => {
    apiFetch<CategoryDto[]>('/api/categories')
      .then((categories) => {
        ...
        setCategory(match);
        if (!match) return;
        return apiFetchWithMeta<DesignSummaryDto[]>(`/api/designs/category/${match.id}?pageSize=50`).then(({ data }) => setDesigns(data));
      })
